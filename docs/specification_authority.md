...
| Prior-vs-posterior comparison summaries and predictive log-density (PSIS-LOO/WAIC) — `REQ-VAL-001` remaining scope | Requirement exists but capability incomplete | `REQ-VAL-001` is approved and substantially implemented, including prior predictive evidence (schema v4, `core.diagnostics.prior_predictive_summary`, `DiagnosticsService.run_prior_predictive_check`). Its own record text explicitly defers these two remaining checks as separately-scoped dependent packages. |
