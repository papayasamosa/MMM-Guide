...
| Historical valuation rate-derivation, posterior join, and forward-assumption contract (`REQ-ECON-003`) | Requirement exists but capability incomplete | Approved 2026-08-28, same brief. WP2B (2026-08-28) delivered Requirements 1-2: `core.outcome_valuation_rates.derive_weekly_value_rates` computes the weekly value-per-outcome rate with the zero-denominator carve-out, surfacing every inconsistent case rather than guessing. Not yet implemented: Requirement 3's draw-level posterior join, Requirement 4's uncertainty propagation (WP2C), and Requirement 5's Scenario Planner forward-assumption contract (WP2G). |
