...
NBT, not GSA. Current UK production onboarding is separately governed by
`REQ-NBT-004`, using the supplied canonical NBT IDs for New, DNA cross-sell,
and Winback; the historical-test 14-day completeness rule is not a production
default. Future brand work is named **Brand-State Mediation Model** with
...
| Governed Search-intent taxonomy, term/query mapping, and parent-child activity lineage (`REQ-SEARCH-004`) | Requirement exists and is partially implemented | The approved Brand/Non-Brand taxonomy, immutable `SearchIntentGroup` records, explicit deeper Non-Brand draft children, ActivityDefinition references, platform-axis validation, parent/child roll-ups, explicit model-grain selection, and project-bundle persistence are implemented in `core.search_intent_taxonomy`, `core.activities`, `pages/10_Channel_Media_Units.py`, and `core.persistence`. Raw term/query mapping and any evidence threshold for separate promotion remain decision-required; no child is auto-invented or auto-fitted. |
| Search granularity capability and multi-axis eligibility contract (`REQ-SEARCH-005`) | Requirement exists and is partially implemented | The implementation preserves the existing activity eligibility contract, adds explicit parent/child model-grain selection and ragged reporting, and keeps deeper-child economics/planning/optimisation unavailable without separate observed/governed cost evidence. The full six-axis capability registry and promotion thresholds remain decision-required. |
| Governed SEO visibility/ranking metric-definition and observation contract (`REQ-SEO-001`) | Requirement exists and is partially implemented | `core.seo_visibility` provides the approved positional/visibility metric, provenance-aware observations, raw or aggregated upload handling, explicit SEO groups, row-aligned windows/masks, persisted standardisation, one coefficient per selected group, and diagnostic-only clicks. SEO remains distinct from organic capture and outside spend CPA/ROI/planning; no controllable SEO intervention is inferred. |
...
- `REQ-SEARCH-004` (`docs/approved_requirements/REQ-SEARCH-004.md`,
  ...
  object-separation/versioning/persistence/validation pattern. The approved
  minimum taxonomy and explicit child draft boundary are now implemented;
  raw term/query mapping and promotion thresholds remain open. Does not approve the
...
- `REQ-SEARCH-005` (`docs/approved_requirements/REQ-SEARCH-005.md`,
  ...
  axes (model/contribution/curve/economics/planning/optimisation). Explicit
  model-grain selection and ragged parent roll-up are implemented; the full
  capability registry and thresholds remain open. Does not approve
...
- `REQ-SEO-001` (`docs/approved_requirements/REQ-SEO-001.md`, approved
  ...
  authority, distinct from organic Search capture. The approved positional/
  visibility implementation and grouped fit boundary are now present. Does
  not approve which metric is used,
