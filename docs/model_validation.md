...
10. Same-response and same-delivery scenarios work. - **Built**: `equivalent_delivery`/`equivalent_response` calculators on Results & Response Curves, plus the Scenario Planner's media-unit planning mode, both built on the same `core.media_units` conversions
