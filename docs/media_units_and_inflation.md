...
- **Scenario planner integration (Phase 3c - built):** the spend plan editor can display/accept
  ...
  "maintain response/delivery under inflation") - `avg_cpa`/`fh_signup_avg_cpa`/`dna_avg_cpa` are
  reported as output metrics on every scenario, not yet optimisation targets themselves. What *is*
  built: an explicit `objective` (`core.optimization.VALID_OBJECTIVES`) - `"fh_gsa"`, `"fh_signups"`,
  `"dna_kits"`, `"weighted_mix"`, `"expected_value"` - with no generic "maximise volume" that would
  silently combine FH GSAs, FH sign-ups and DNA kits. Marginal CPA at the scenario level was
  considered and deliberately not built: the planner always conserves total budget, so there's no net
  spend change to compute a marginal CPA against - the *metric-aware average* CPA (FH GSAs, FH
  sign-ups and DNA kits kept separate) of the current vs. optimised allocation is the well-defined
  metric shown instead (`docs/decision_log.md`).
