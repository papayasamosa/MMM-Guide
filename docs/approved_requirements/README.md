...
- `REQ-OPT-*` — optimiser objective-kind and constraint-kind vocabulary (multi-objective goals, month x channel constraint types) — zero implementation, blocked pending a future decision package for the sequential-kernel tractability and schema-representation questions
- `REQ-DATASUPPORT-*` — evidence-based per-channel data-support/identifiability classification, consolidating existing coverage/identifiability diagnostics — zero implementation, no numeric threshold approved
