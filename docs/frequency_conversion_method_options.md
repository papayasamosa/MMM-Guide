...
| 2 | Reconciliation | Exact by construction **only when sub-periods nest cleanly inside the source period** (e.g. an artificial equal-length split). Review finding: this repository's actual monthly-to-weekly case uses real calendar weeks, which routinely straddle a month boundary — a boundary week cannot simultaneously receive an "identical amount to every week" (the constancy assumption in row 1) and still reconcile exactly, since only part of that week's days fall inside the source month. Exact reconciliation at calendar-week boundaries requires an explicit day-count-weighted-overlap convention for boundary weeks specifically, which breaks the "identical amount every week" framing for those particular weeks — a future approval must pick one (exact reconciliation with non-uniform boundary weeks, or uniform weeks with a small, bounded, explicitly-documented reconciliation error at boundaries), not assume both hold everywhere | Exact by construction if the benchmarking constraint is enforced (Chow-Lin/Denton explicitly solve for this, including at non-nested boundaries, since the constraint is solved over whatever sub-periods actually exist); approximate otherwise | Not reconciled — summing the repeated value across sub-periods over-counts the source total by a factor equal to the sub-period count | N/A — no disaggregated series to reconcile |
| 3 | Publication-lag behaviour | Review finding: none of A/B/C is safe merely because `publication_lag_periods=0` is configured — the source-period *total itself does not exist* until the full source period has elapsed, regardless of lag. All three require `as_of` to be at or after the source period's own end (plus any additional publication lag) before they may be used at all; `core.frequency_alignment.check_publication_leakage` already encodes exactly this (an `as_of` before `reconstructed_period_end` is leakage even at zero lag) — this row describes behaviour *given* that check has already passed, not an exemption from it. Once past that point: safe — needs only the already-published source-period total | As above (needs the source total once elapsed/published), plus needs the indicator series' own values at the same sub-periods; if the indicator itself has a shorter publication lag than the flow variable, no additional leakage; if the indicator is itself lagged or revised, must be checked independently | As above (needs the source total once elapsed/published); additionally overstates value in every sub-period until the *next* source observation is known, which is itself a leakage risk for periods within the current, still-unclosed *next* source window | Safe — no interpolation to leak from, but still bound by the source's own native-frequency publication timing |
...
ownership) selects **at most one** candidate above per variable class (matching Section 1),
or requests a different candidate not listed here. This is not an arbitrary restriction:
`core.frequency_alignment.resolve_conversion_method` returns the first *approved* registry
entry it finds for a class when an `AlignmentSpecification` omits `method_id` (review
finding) — approving more than one method for the same class without also requiring every
request to name `method_id` explicitly would make the selected conversion depend on registry
insertion order, not on a governed choice. If a reviewer genuinely wants more than one
approved method per class (e.g. one for markets with an available higher-frequency indicator,
another as a fallback where none exists), the resulting requirement record must also mandate
that every `AlignmentSpecification` for that class sets `method_id` explicitly - this document
does not itself make that call.

That selection becomes a new, separately-scoped approved requirement record (e.g.
`REQ-FREQ-001`) which:

- names the approved method per variable class (or, if more than one, the explicit
  `method_id`-selection rule above),
