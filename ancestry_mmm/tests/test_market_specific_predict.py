...
class TestExtractMarketSpecificPosteriorParams:
    ...
    def test_at_still_selects_correctly_for_market_and_segment_indexed_fields(
        self, trace, meta
    ):
...
class TestSteadyStateSegmentResponseMarketSpecific:
    def test_zero_spend_matches_hand_computed_baseline(self, meta, params):
        ...
        expected_new = np.exp(
            params.intercept["New"] + 0.0 + params.trend_coef["New"] * 1.0
        )
        expected_dna = np.exp(
            params.intercept["DNA_CrossSell"]
            + 0.0
            + params.trend_coef["DNA_CrossSell"] * 1.0
        )
    ...
    def test_different_markets_give_different_responses(self, meta, params):
        uk = steady_state_segment_response_market_specific(
            "UK", {"TV": 500.0, "DNA_Media": 200.0}, meta, params
        )
        au = steady_state_segment_response_market_specific(
            "AU", {"TV": 500.0, "DNA_Media": 200.0}, meta, params
        )
        assert uk["New"] != pytest.approx(au["New"])

    def test_more_spend_increases_response(self, meta, params):
        low = steady_state_segment_response_market_specific(
            "UK", {"TV": 10.0}, meta, params
        )
        high = steady_state_segment_response_market_specific(
            "UK", {"TV": 5000.0}, meta, params
        )
...
class TestGenerateMarketChannelCurve:
    ...
    def test_zero_spend_gives_zero_response(self, meta, params):
        df = generate_market_channel_curve(
            "UK", "TV", meta, params, spend_range=np.array([0.0, 100.0, 500.0])
        )
    ...
    def test_overall_response_is_sum_of_segment_responses(self, meta, params):
        df = generate_market_channel_curve(
            "UK", "DNA_Media", meta, params, spend_range=np.array([0.0, 250.0, 1000.0])
        )
    ...
    def test_dna_channel_halo_scales_non_dna_segment_response(self, meta, params):
        ...
        df = generate_market_channel_curve(
            "UK", "DNA_Media", meta, params, spend_range=np.array([500.0])
        )
        row = df.iloc[0]
    ...
    def test_response_increases_with_spend(self, meta, params):
        df = generate_market_channel_curve(
            "UK", "TV", meta, params, spend_range=np.array([0.0, 500.0, 5000.0])
        )
...
class TestGenerateMarketChannelCurveDirectDnaSegments:
    ...
    def test_dna_kit_segment_gets_full_response_not_halo_shrunk(
        self, meta_with_dna_kit_segment, params_with_dna_kit_segment
    ):
        df = generate_market_channel_curve(
            "UK",
            "DNA_Media",
            meta_with_dna_kit_segment,
            params_with_dna_kit_segment,
            spend_range=np.array([500.0]),
        )
        row = df.iloc[0]
        raw = (
            params_with_dna_kit_segment.beta["UK"]["New Customer"]["DNA_Media"]
            * row["saturation"]
        )
    ...
    def test_ordinary_non_direct_segment_is_still_halo_shrunk(
        self, meta_with_dna_kit_segment, params_with_dna_kit_segment
    ):
        df = generate_market_channel_curve(
            "UK",
        ...
        row = df.iloc[0]
        raw_new = (
            params_with_dna_kit_segment.beta["UK"]["New"]["DNA_Media"]
            * row["saturation"]
        )
        assert row["New_response"] == pytest.approx(
            raw_new * params_with_dna_kit_segment.pathway_strength["New"]["DNA_Media"]
        )
