...
def params() -> FHMarketSpecificPosteriorParams:
    return FHMarketSpecificPosteriorParams(
        ...
        pathway_strength=pathway_strength_from_flat(
            {"New": 0.15, "DNA_CrossSell": 1.0, "New Customer": 0.2}, "DNA_Media"
        ),
...
class TestComputeShapleyContributionsMarketSpecific:
    def test_contributions_sum_to_mu_total_for_every_row_and_segment(
        self, frame, meta, params
    ):
        contributions = compute_shapley_contributions_market_specific(
    ...
    def test_contributions_differ_by_market_since_beta_and_hill_k_are_market_indexed(
        self, frame, meta, params
    ):
        contributions = compute_shapley_contributions_market_specific(
            frame, meta, params, n_permutations=50
        )
    ...
    def test_dna_kit_segment_channel_contribution_uses_full_beta_not_halo_shrunk(
        ...
        contributions_direct = compute_shapley_contributions_market_specific(
            frame, meta, params, n_permutations=20
        )
        ...
        contributions_shrunk = compute_shapley_contributions_market_specific(
...
class TestSegmentChannelMarketSummary:
    def test_has_one_row_per_market_channel_segment_combination(
        self, frame, meta, params
    ):
    ...
    def test_spend_is_the_markets_own_channel_spend_not_shared_across_markets(
        self, frame, meta, params
    ):
        summary = segment_channel_market_summary(frame, meta, params, n_permutations=20)
        uk_tv_spend = summary[
            (summary["market"] == "UK") & (summary["channel"] == "TV")
        ]["spend"].iloc[0]
        au_tv_spend = summary[
            (summary["market"] == "AU") & (summary["channel"] == "TV")
        ]["spend"].iloc[0]
...
class TestTotalContributionMarketSpecific:
    def test_default_aggregates_across_markets_to_one_row_per_channel(
        self, frame, meta, params
    ):
        contributions = compute_shapley_contributions_market_specific(
            frame, meta, params, n_permutations=20
        )
        total = total_contribution_market_specific(
    ...
    def test_by_market_keeps_market_as_a_grouping_key(self, frame, meta, params):
        ...
        total = total_contribution_market_specific(
            frame, meta, params, contributions, ltv=None, by_market=True
        )
        assert len(total) == len(MARKETS) * len(CHANNELS)

    def test_total_spend_does_not_double_count_across_dna_kit_and_fh_segment_rows(
        self, frame, meta, params
    ):
    ...
    def test_outcome_ids_filter_excludes_dna_kit_outcome_from_the_total(
        ...
        total_all = total_contribution_market_specific(
        ...
        assert total_fh_only.set_index("channel").loc[
            "DNA_Media", "spend"
        ] == pytest.approx(total_all.set_index("channel").loc["DNA_Media", "spend"])
    ...
    def test_ltv_weighting_scales_value_contribution_per_segment(
        self, frame, meta, params
    ):
        contributions = compute_shapley_contributions_market_specific(
            frame, meta, params, n_permutations=20
        ...
        total = total_contribution_market_specific(
            frame, meta, params, contributions, ltv=ltv
        )
        summary = segment_channel_market_summary(
            frame, meta, params, contributions, ltv=ltv
        )
        expected = summary.groupby("channel")["value_contribution"].sum()
        for ch in CHANNELS:
            assert total.set_index("channel").loc[
                ch, "value_contribution"
...
class TestShapleyMarketSpecificDirectHaloSeparation:
    ...
    def test_changing_halo_lag_does_not_alter_the_direct_kit_contribution(self):
        ...
        c2 = compute_shapley_contributions_market_specific(
            frame, self._meta(dna_lag_weeks=2), params, n_permutations=5
        )
        c5 = compute_shapley_contributions_market_specific(
