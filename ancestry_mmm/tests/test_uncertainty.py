...
def market_trace() -> az.InferenceData:
    ...
    posterior = {
        ...
        * (1 + rng.normal(0, 0.05, size=(n_chain, n_draw, 2, 2))),
        "hill_S": const([1.1, 1.0]),
...
class TestSampleDrawIndices:
    ...
    def test_is_deterministic_given_the_same_seed(self, trace):
        assert sample_draw_indices(trace, n_draws=10, seed=7) == sample_draw_indices(
            trace, n_draws=10, seed=7
        )

    def test_different_seeds_can_give_different_samples(self, trace):
        assert sample_draw_indices(trace, n_draws=10, seed=1) != sample_draw_indices(
            trace, n_draws=10, seed=2
        )
...
class TestGenerateChannelCurveWithUncertainty:
    def test_lower_le_mean_le_upper_at_every_spend_point(self, meta, trace):
        df = generate_channel_curve_with_uncertainty(
            "TV_Brand", meta, trace, n_draws=20, seed=1, n_points=8
        )
        assert np.all(
            df["overall_response_lower"] <= df["overall_response_mean"] + 1e-9
        )
        assert np.all(
            df["overall_response_mean"] <= df["overall_response_upper"] + 1e-9
        )
    ...
    def test_raising_n_draws_does_not_raise_when_posterior_is_smaller_than_requested(
        self, meta, trace
    ):
        # Posterior has only 40 (chain, draw) pairs total.
        df = generate_channel_curve_with_uncertainty(
            "TV_Brand", meta, trace, n_draws=1000, seed=1, n_points=5
        )
        assert len(df) == 5

    def test_no_warnings_raised_despite_the_zero_spend_undefined_cpa_point(
        self, meta, trace
    ):
        with warnings.catch_warnings():
            warnings.simplefilter("error")
            generate_channel_curve_with_uncertainty(
                "TV_Brand", meta, trace, n_draws=10, seed=1, n_points=5
            )
...
class TestGenerateMarketChannelCurveWithUncertainty:
    def test_lower_le_mean_le_upper_at_every_spend_point(self, meta, market_trace):
        df = generate_market_channel_curve_with_uncertainty(
            "UK", "TV_Brand", meta, market_trace, n_draws=20, seed=1, n_points=8
        )
        assert np.all(
            df["overall_response_lower"] <= df["overall_response_mean"] + 1e-9
        )
    ...
    def test_different_markets_give_different_mean_curves(self, meta, market_trace):
        uk = generate_market_channel_curve_with_uncertainty(
            "UK", "TV_Brand", meta, market_trace, n_draws=20, seed=1, n_points=5
        )
        au = generate_market_channel_curve_with_uncertainty(
            "AU", "TV_Brand", meta, market_trace, n_draws=20, seed=1, n_points=5
        )
...
class TestEvaluateScenarioWithUncertainty:
    ...
    def reference_context(self):
        return {
            "2024-01": {
                "trend": 1.0,
                "fourier": np.zeros(4),
                "promo": {s: 0.0 for s in OUTCOME_IDS},
                "controls": {},
                "outcome_controls": {},
            }
        }

    def test_summary_has_lower_le_mean_le_upper_for_value(
    ...
    def test_paired_baseline_comparison_gives_prob_one_when_proposed_strictly_dominates(
        ...
        result = evaluate_scenario_with_uncertainty(
            ...
            approval=approval,
            governance_mode="exploratory",
            baseline_spend_plan=lower_spend,
            **IDENTITY,
        )
        assert result["prob_outperforms_baseline"] == pytest.approx(1.0)

    def test_paired_baseline_comparison_gives_prob_zero_when_reversed(
        self, meta, market_trace, approval, reference_context
    ):
        higher_spend = {"2024-01": {"TV_Brand": 5000.0, "DNA_Media": 2000.0}}
        lower_spend = {"2024-01": {"TV_Brand": 10.0, "DNA_Media": 5.0}}
        ...
        result = evaluate_scenario_with_uncertainty(
            ...
            approval=approval,
            governance_mode="exploratory",
            baseline_spend_plan=higher_spend,
            **IDENTITY,
        )
        assert result["prob_outperforms_baseline"] == pytest.approx(0.0)

    def test_summary_carries_product_aware_and_total_value_columns(
        self, meta, market_trace, approval, reference_context
    ):
        ...
        assert np.all(
            summary["dna_avg_cpa_mean"].isna()
        )  # no kit-only segments in this fixture
        assert np.all(
            summary["total_value_lower"] <= summary["total_value_mean"] + 1e-9
        )
        assert np.all(
            summary["total_value_mean"] <= summary["total_value_upper"] + 1e-9
        )
...
class TestEvaluateScenarioWithUncertaintyGovernance:
    ...
    def test_accepts_outcome_governance_kwargs_without_typeerror(
        self, meta, market_trace, approval, reference_context
    ):
        spend_plan = {"2024-01": {"TV_Brand": 1000.0, "DNA_Media": 200.0}}
    ...
    def test_official_mode_with_matching_approval_succeeds(
        ...
        result = evaluate_scenario_with_uncertainty(
            ...
            approval=approval,
            planning_objective=planning_objective,
            outcome_approvals=[gsa_approval],
            governance_mode="official",
            **IDENTITY,
        )
        assert "summary" in result
    ...
    def test_rejected_outcome_approval_blocks(
        ...
        with pytest.raises(OutcomeApprovalBlockedError):
            evaluate_scenario_with_uncertainty(
                ...
                approval=approval,
                planning_objective=planning_objective,
                outcome_approvals=[rejected_approval],
                governance_mode="official",
                **IDENTITY,
            )
    ...
    def test_expired_outcome_approval_blocks(
        ...
        with pytest.raises(OutcomeApprovalBlockedError):
            evaluate_scenario_with_uncertainty(
                ...
                approval=approval,
                planning_objective=planning_objective,
                outcome_approvals=[expired_approval],
                governance_mode="official",
                **IDENTITY,
            )
