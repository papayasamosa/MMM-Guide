...
class TestMakeEntriesRequiresMatchingApproval:
    ...
    def test_mismatched_data_fingerprint_raises(
        self, shared_meta, shared_params, approval
    ):
        current = dict(IDENTITY)
...
class TestMakeEntriesValidation:
    ...
    def test_market_specific_without_evidence_tiers_raises(
        self, market_specific_meta, market_specific_params, approval
    ):
...
class TestMakeEntriesSharedModel:
    def test_one_entry_per_channel_segment_plus_overall(
        self, shared_meta, shared_params, approval
    ):
    ...
    def test_overall_beta_is_sum_of_segment_betas(
        self, shared_meta, shared_params, approval
    ):
        entries = make_entries(
            shared_meta,
        ...
        overall = next(
            e
            for e in entries
            if e.channel == "TV_Brand" and e.segment_or_overall == OVERALL
        )
        expected = sum(shared_params.beta[s]["TV_Brand"] for s in SEGMENTS)
        assert overall.beta == pytest.approx(expected)

    def test_dna_channel_carries_halo_strength_non_dna_channel_does_not(
        self, shared_meta, shared_params, approval
    ):
        entries = make_entries(
            shared_meta,
        ...
        dna_entry = next(
            e
            for e in entries
            if e.channel == "DNA_Media" and e.segment_or_overall == "New"
        )
        non_dna_entry = next(
            e
            for e in entries
            if e.channel == "TV_Brand" and e.segment_or_overall == "New"
        )
    ...
    def test_entries_retain_all_model_identifiers(
        self, shared_meta, shared_params, approval
    ):
        entries = make_entries(
            shared_meta,
...
class TestMakeEntriesMarketSpecificModel:
    EVIDENCE_TIERS = {
        ...
        "Australia": {
            "TV_Brand": "Partially pooled",
            "DNA_Media": "Transferred estimate",
        },
    }

    def test_one_entry_per_market_channel_segment_plus_overall(
        self, market_specific_meta, market_specific_params, approval
    ):
        entries = make_entries(
            market_specific_meta,
    ...
    def test_curve_status_comes_from_evidence_tiers_per_market_and_channel(
        self, market_specific_meta, market_specific_params, approval
    ):
        entries = make_entries(
            market_specific_meta,
        ...
        uk_tv = next(
            ...
            for e in entries
            if e.market == "UK"
            and e.channel == "TV_Brand"
            and e.segment_or_overall == "New"
        )
        au_dna = next(
            e
            for e in entries
            if e.market == "Australia"
            and e.channel == "DNA_Media"
            and e.segment_or_overall == "New"
        )
        assert uk_tv.curve_status == "Locally estimated"
        assert au_dna.curve_status == "Transferred estimate"

    def test_currency_by_market_is_applied_per_market(
        self, market_specific_meta, market_specific_params, approval
    ):
        entries = make_entries(
            market_specific_meta,
            ...
            model_type="market_specific",
            evidence_tiers=self.EVIDENCE_TIERS,
            currency_by_market={"UK": "GBP", "Australia": "AUD"},
            **IDENTITY,
        )
    ...
    def test_missing_currency_for_a_market_defaults_to_none(
        self, market_specific_meta, market_specific_params, approval
    ):
        entries = make_entries(
            market_specific_meta,
    ...
    def test_beta_differs_between_markets_when_params_do(
        self, market_specific_meta, approval
    ):
        markets = ["UK", "Australia"]
...
class TestLegacyEntryExpansion:
    LEGACY_DICT = {
        ...
        "data_window": ("2023-01-01", "2023-12-31"),
        "markets": ["UK"],
        "segments": SEGMENTS,
        "channels": CHANNELS,
        "dna_channels": ["DNA_Media"],
        ...
        "beta": {
            "New": {"TV_Brand": 0.1, "DNA_Media": 0.05},
            "DNA_CrossSell": {"TV_Brand": 0.02, "DNA_Media": 0.2},
        },
        "halo_strength": {"New": 0.15, "DNA_CrossSell": 1.0},
        "promo_coef": {"New": 0.1, "DNA_CrossSell": 0.2},
...
def test_curve_bank_is_append_only_and_versioned(
    tmp_path, shared_meta, shared_params, approval
):
    ...
    assert {e.entry_id for e in loaded} == {e.entry_id for e in entries_1} | {
        e.entry_id for e in entries_2
    }
...
def test_entries_to_dataframe_includes_approval_and_status_columns(
    shared_meta, shared_params, approval
):
    entries = make_entries(
        shared_meta,
        shared_params,
...
class TestCompareToTest:
    def test_agrees_when_inside_credible_interval(self):
        assert (
            compare_to_test(model_estimate=2.5, test_estimate=2.0, test_ci=(2.0, 3.0))
            == "agrees"
        )

    def test_diverges_when_outside_credible_interval(self):
        assert (
            compare_to_test(model_estimate=5.0, test_estimate=2.0, test_ci=(2.0, 3.0))
            == "diverges"
        )

    def test_falls_back_to_tolerance_percent_without_ci(self):
        assert (
            compare_to_test(model_estimate=1.05, test_estimate=1.0, tolerance_pct=10.0)
            == "agrees"
        )
        assert (
            compare_to_test(model_estimate=1.5, test_estimate=1.0, tolerance_pct=10.0)
            == "diverges"
        )


class TestMakeMediaUnitEntries:
    def test_only_mirrors_channels_with_a_media_unit_mapping(
        self, shared_meta, shared_params, approval
    ):
        entries = make_entries(
            shared_meta,
    ...
    def test_mirrored_entries_carry_the_cost_context(
        self, shared_meta, shared_params, approval
    ):
        entries = make_entries(
            shared_meta,
    ...
    def test_mirrored_entries_carry_the_same_curve_parameters_as_the_source(
        self, shared_meta, shared_params, approval
    ):
        entries = make_entries(
            shared_meta,
        ...
        source = next(
            e
            for e in entries
            if e.channel == "TV_Brand"
            and e.segment_or_overall == mirrored[0].segment_or_overall
        )
        assert mirrored[0].beta == pytest.approx(source.beta)
        assert mirrored[0].hill_K == pytest.approx(source.hill_K)
        assert (
            mirrored[0].entry_id != source.entry_id
        )  # a distinct record, not the same one mutated

    def test_no_mapping_means_no_mirrored_entries(
        self, shared_meta, shared_params, approval
    ):
        entries = make_entries(
            shared_meta,
    ...
    def test_does_not_mirror_an_already_media_unit_entry(
        self, shared_meta, shared_params, approval
    ):
        entries = make_entries(
            shared_meta,
    ...
    def test_matches_by_market_for_a_market_specific_run(
        self, market_specific_meta, market_specific_params, approval
    ):
        evidence_tiers = {
            "UK": {"TV_Brand": "Locally estimated", "DNA_Media": "Locally estimated"},
