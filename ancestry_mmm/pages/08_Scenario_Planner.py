...
st.markdown("### Spend plan (monthly, by channel) - edit directly for manual mode")
edited = st.data_editor(st.session_state[plan_key], width="stretch", key=f"editor_{plan_key}")
...
with tab_constrained:
    ...
    with st.expander("➕ Add a constraint"):
        kind = st.selectbox("Constraint type", ["locked_cell", "channel_total", "month_total", "bounded_movement", "min_spend_floor"])
        ch = st.selectbox("Channel (if applicable)", ["(any)"] + meta.channels, key="c_channel")
    ...
    for i, c in enumerate(st.session_state["scenario_constraints"]):
        ...
        c1.markdown(f"**{i+1}.** {c.kind} - channel={c.channel or 'any'}, month={c.month or 'any'}, value={c.value}, max_pct_move={c.max_pct_move}")
    ...
    if result:
        ...
        st.dataframe(pd.DataFrame(result["spend_plan"]).T, width="stretch")
        st.dataframe(result["predicted"], width="stretch")
...
scenarios = get_state("scenarios") or []
if scenarios:
    st.dataframe(compare_scenarios(scenarios), width="stretch")
else:
    st.info("No scenarios saved yet.")

st.markdown("---")
if st.button("Continue to Project Export & Handover →", type="primary"):
