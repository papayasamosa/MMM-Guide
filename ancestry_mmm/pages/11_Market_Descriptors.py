...
for market in spec.markets:
    market_df = (
        df[df[spec.market_col] == market] if spec.market_col in df.columns else df
    )
    n_obs = len(market_df)
    date_range = (
        f"{format_date(market_df[spec.date_col].min())} to {format_date(market_df[spec.date_col].max())}"
        if n_obs and spec.date_col in market_df.columns
        else "(no data)"
    )
    ...
    with st.expander(f"Market: {market} - {quality}", expanded=len(spec.markets) == 1):
        with st.container():
            ...
            c4.metric(
                "Media-unit coverage",
                f"{mapped_channels}/{len(spec.channels)} channels",
            )
            st.caption(
                f"Channels: {', '.join(spec.channels) or '(none)'}. Data quality: {quality}."
            )
