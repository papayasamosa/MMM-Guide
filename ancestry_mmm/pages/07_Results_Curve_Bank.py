...
if model_type == "market_specific":
    st.markdown("---")
    st.info(
        "Shapley attribution and saving to the curve bank aren't available yet for market-specific "
        "models - they're built around a single shared curve per channel and would misread a "
        "market-specific fit. This is planned for a later phase (see docs/curve_bank.md). "
        "In the meantime, explore each market's own channel curves below."
    )
    ...
    channel = c2.selectbox("Channel", meta.channels)

    curve_df = generate_market_channel_curve(market, channel, meta, params)
    st.plotly_chart(
        create_response_curve(
            curve_df["spend"].to_numpy(), curve_df["overall_response"].to_numpy(), f"{market} - {channel}",
        ),
        width="stretch",
    )
