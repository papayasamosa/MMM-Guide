...
def extract_posterior_params(trace: az.InferenceData, meta: FHModelMeta) -> FHPosteriorParams:
    ...
    hill_K = mean_by_coord("hill_K", "channel", meta.channels)
    ...
    beta = {
        s: {c: float(beta_mean.sel(segment=s, channel=c).values) for c in meta.channels}
    ...
    gamma_fourier = {
        s: gamma_fourier_mean.sel(segment=s).values for s in meta.segments
    }
