...
def _week_start(value: Any, *, anchor: str = "sunday") -> pd.Timestamp:
    timestamp = pd.Timestamp(value).normalize()
    anchor_key = str(anchor).strip().lower()
    if anchor_key not in {"sunday", "monday"}:
        raise FrequencyConversionError(
            f"weekly source anchor must be Sunday or Monday, got {anchor!r}"
        )
    days_since_anchor = (
        (int(timestamp.weekday()) + 1) % 7
        if anchor_key == "sunday"
        else int(timestamp.weekday())
    )
    return timestamp - pd.Timedelta(days=days_since_anchor)
...
def _frequency_period_bounds(
    value: Any, frequency: str, *, week_anchor: str = "sunday"
    ...
    if frequency_key == "weekly":
        start = _week_start(timestamp, anchor=week_anchor)
    ...
    *,
    ...
    for row_number, row in frame.iterrows():
        ...
        source_start, source_end = _frequency_period_bounds(
            row[date_col],
            spec.native_frequency,
            week_anchor=str(spec.parameters.get("week_anchor", "sunday")),
        )
        if pd.isna(value):
            continue
...
def _release_date(
    ...
    *,
    ...
    _, source_end = _frequency_period_bounds(
        row[date_col],
        spec.native_frequency,
        week_anchor=str(spec.parameters.get("week_anchor", "sunday")),
    )
    ...
    frequency_alias = {
        "daily": "D",
        # A Sunday-start week ends on Saturday.  ``W-SAT`` is used only for
        # release-lag arithmetic; the actual target labels are generated by
        # ``_week_start`` so they remain Sunday dates.
        "weekly": "W-SAT",
        "monthly": "M",
        "quarterly": "Q",
            ...
            if not eligible.empty:
                ...
                output[f"{value_col}__age_periods"] = int(
                    (
                        ...
                        - _week_start(
                            latest[date_col],
                            anchor=str(spec.parameters.get("week_anchor", "sunday")),
                        )
                    ).days
                    // 7
                )
...
def _native_cadence_only(
    ...
    *,
    ...
    typed[date_col] = (
        typed[date_col].map(
            lambda value: _week_start(
                value, anchor=str(spec.parameters.get("week_anchor", "sunday"))
            )
        )
...
def _weekly_anchor_alignment(
    ...
    *,
    ...
    """Align a native weekly source to Sunday labels without moving facts.

    A Sunday-anchored observation belongs to that same Sunday model week.  A
    Monday-anchored observation covers Monday-Sunday and is assigned to the
    following Sunday model week; it is never shifted backwards into the
    preceding Sunday week.  Source dates and values are retained in the
    lineage evidence.
    """

    anchor = str(spec.parameters.get("week_anchor", "sunday")).strip().lower()
    if anchor not in {"sunday", "monday"}:
        raise FrequencyConversionError(
            "weekly_anchor_alignment requires week_anchor Sunday or Monday"
        )
    rows: list[dict[str, Any]] = []
    target_set = set(target_periods)
    for _, source in frame.iterrows():
        source_date = pd.Timestamp(source[date_col]).normalize()
        if anchor == "sunday":
            target_start = _week_start(source_date)
        else:
            source_end = source_date + pd.Timedelta(days=6)
            target_start = _week_start(source_end)
        if target_start not in target_set or pd.isna(source[value_col]):
            continue
        row = {date_col: target_start, value_col: source[value_col]}
        if market_col:
            row[market_col] = source[market_col]
        rows.append(row)
    output = pd.DataFrame(
        rows, columns=[date_col] + ([market_col] if market_col else []) + [value_col]
    )
    ...
    evidence = _common_evidence(
        ...
        output_rows=len(output),
        method_notes=(
            "Native weekly observations retain their factual dates; Sunday "
            "sources map to the same Sunday week and Monday sources map to "
            "the following Sunday week to avoid backward look-ahead."
        ),
    )
    evidence["source_week_anchor"] = anchor
    return ConversionExecution(output, evidence)
...
def _event_alignment(
    ...
    *,
    ...
    evidence = _common_evidence(
        ...
        output_rows=len(output),
        method_notes=(
            "Point events are placed on their containing Sunday week; duration "
...
_METHOD_CATALOGUE = {
    ...
    "rate_index": ("release_aware_locf", "weekly_anchor_alignment"),
...
def validate_conversion_spec(spec: AlignmentSpecification) -> None:
    ...
    if method.method_id == "calendar_overlap_allocation":
        if set(parameters) - {"source_column", "week_anchor"}:
            raise FrequencyConversionError(
                "calendar_overlap_allocation accepts only source_column and week_anchor"
            )
    elif method.method_id == "release_aware_locf":
        allowed = {"release_basis", "source_column", "week_anchor"}
    ...
    elif method.method_id == "weekly_anchor_alignment":
        if set(parameters) - {"week_anchor", "source_column"}:
            raise FrequencyConversionError(
                "weekly_anchor_alignment accepts only week_anchor and source_column"
            )
        if str(parameters.get("week_anchor", "")).strip().lower() not in {
            "sunday",
            "monday",
        }:
            raise FrequencyConversionError(
...
def ensure_approved_frequency_methods() -> None:
    ...
    _register(
        ...
        description="Align weekly rate/index observations to Sunday model weeks without backward shifting.",
        executor=_weekly_anchor_alignment,
    )
    _register(
