...
class CurveReferenceContext:
    ...
    def __post_init__(self) -> None:
        ...
        if self.counterfactual_value < 0 or not np.isfinite(self.counterfactual_value):
            raise ValueError("counterfactual_spend must be finite and non-negative")
        if any(
            value < 0 or not np.isfinite(value)
            for value in self.other_channel_media_input.values()
        ):
            raise ValueError(
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "controls": dict(self.controls),
            "outcome_controls": {
                key: dict(value) for key, value in self.outcome_controls.items()
            },
            "other_channel_media_input": dict(self.other_channel_media_input),
    ...
    def from_dict(cls, values: Mapping[str, object]) -> "CurveReferenceContext":
        ...
        if "other_channel_media_input" not in payload:
            payload["other_channel_spend"] = payload.pop("other_channel_spend")
        if "counterfactual_value" not in payload:
            payload["counterfactual_spend"] = payload.pop("counterfactual_spend", 0.0)
...
class PortfolioPerturbation:
    ...
    def __post_init__(self) -> None:
        ...
        if not len(values) or np.any(~np.isfinite(values)) or np.any(values < 0):
            raise ValueError(
                "allocation_direction must contain finite non-negative shares"
            )
...
def _normalise_support(
    meta: FHModelMeta,
    support_by_market_channel: Optional[Mapping[Tuple[str, str], object]],
    *,
    curve_type: str,
    legacy: bool,
    ...
    for market in meta.markets:
        for channel in meta.channels:
            supplied_value = supplied.get((market, channel))
            if isinstance(supplied_value, MediaInputSupport):
                if curve_type != "model_input":
                    raise ValueError(
                        "Monetary curves require MonetarySpendSupport; "
                        "MediaInputSupport cannot be interpreted as currency"
                    )
                values = {
                    "current_spend": supplied_value.current,
                    "observed_spend_min": supplied_value.observed_min,
                    "observed_spend_max": supplied_value.observed_max,
                    "planning_spend_min": supplied_value.planning_min,
                    "planning_spend_max": supplied_value.planning_max,
                    "current_spend_method": supplied_value.current_method,
                    "current_spend_reference_period_start": (
                        supplied_value.effective_period_start
                    ),
                    "current_spend_reference_period_end": (
                        supplied_value.effective_period_end
                    ),
                    "support_axis_type": "model_input",
                    "support_unit": supplied_value.unit,
                    "support_provenance": supplied_value.provenance,
                }
            elif isinstance(supplied_value, MonetarySpendSupport):
                if curve_type != "monetary":
                    raise ValueError("Model-input curves require MediaInputSupport")
                values = {
                    "current_spend": supplied_value.current_local,
                    "observed_spend_min": supplied_value.observed_local_min,
                    "observed_spend_max": supplied_value.observed_local_max,
                    "planning_spend_min": supplied_value.planning_local_min,
                    "planning_spend_max": supplied_value.planning_local_max,
                    "current_spend_method": supplied_value.current_method,
                    "current_spend_reference_period_start": (
                        supplied_value.effective_period_start
                    ),
                    ...
                    "support_axis_type": "monetary",
                    "support_unit": supplied_value.local_currency,
                    "support_provenance": supplied_value.provenance,
                    "current_media_input": supplied_value.current_media_input,
                    "observed_media_input_min": (
                        supplied_value.observed_media_input_min
                    ),
                    "observed_media_input_max": (
                        supplied_value.observed_media_input_max
                    ),
                    "planning_media_input_min": (
                        supplied_value.planning_media_input_min
                    ),
                    "planning_media_input_max": (
                        supplied_value.planning_media_input_max
                    ),
                }
            elif supplied_value is not None and legacy:
                values = dict(supplied_value)
                values["support_axis_type"] = "legacy_ambiguous"
            elif supplied_value is not None:
                raise TypeError(
                    "Explicit curves require typed MediaInputSupport or "
                    "MonetarySpendSupport"
                )
            else:
                values = {}
            ...
            result[(market, channel)] = {
                ...
                "support_axis_type": values.get("support_axis_type"),
                "support_unit": values.get("support_unit"),
                "support_provenance": values.get("support_provenance"),
                **{
...
def support_from_model_frame(
    ...
    *,
    ...
    media_input_specs: Mapping[Tuple[str, str], MediaInputSpec],
) -> Dict[Tuple[str, str], MediaInputSupport]:
    """Derive support from ``X_media`` in fitted model-input units only."""
    if current_spend_method not in CURRENT_SPEND_METHODS:
        raise ValueError(f"Unsupported current-spend method '{current_spend_method}'")
    media = np.asarray(frame["X_media"], dtype=float)
    market_idx = np.asarray(frame.get("market_idx", np.zeros(len(media))), dtype=int)
    dates = pd.to_datetime(frame.get("dates", np.arange(len(media))), errors="coerce")
    result = {}
    for market_pos, market in enumerate(meta.markets):
        ...
        for channel_pos, channel in enumerate(meta.channels):
            ...
            spec = media_input_specs.get((market, channel))
            if spec is None:
                raise ValueError(f"Missing MediaInputSpec for {market}/{channel}")
            result[(market, channel)] = MediaInputSupport(
                market=market,
                channel=channel,
                unit=spec.unit,
                ...
                planning_min=float(np.nanmin(values)),
                planning_max=float(np.nanmax(values)),
                current_method=current_spend_method,
                source=spec.source or spec.column,
                provenance="model_frame:X_media",
...
def reference_context_from_model_frame(
    ...
    *,
    ...
    if control_names:
        control_values = np.asarray(frame["X_controls"], dtype=float)[mask].mean(axis=0)
        controls = {
...
def _economic_values(
    *,
    ...
    marginal_status = ECONOMICS_OK if marginal_ok else ECONOMICS_NEAR_ZERO_MARGINAL
    if not units_valid:
    ...
    return {
        ...
        "roi_status": (ECONOMICS_MISSING_VALUE if not value_ok else status),
    }
...
def generate_canonical_curve_draws(
    *,
    ...
    media_input_specs: Optional[Mapping[Tuple[str, str], MediaInputSpec]] = None,
    cost_mappings: Optional[Mapping[Tuple[str, str], MediaCostMapping]] = None,
    ...
    if governed_costs and cost_mapping_registry is not None:
        raise ValueError("Supply cost_mappings or cost_mapping_registry, not both")
    if legacy_monetary:
        warnings.warn(
            "curve_type=None is deprecated and ungoverned; migrate to an "
            "explicit model_input or monetary curve before G2A.4",
            DeprecationWarning,
            stacklevel=2,
        )
    ...
    if not legacy_monetary:
        ...
        if missing_specs:
            raise ValueError(
                f"Explicit media-input metadata is required for {sorted(missing_specs)}"
            )
        ...
        for market in meta.markets:
            ...
            for channel in meta.channels:
                ...
                monetary_fingerprint = (
                    monetary_governance_fingerprint(
                        cost_mappings=(cost_mapping.to_dict() if cost_mapping else {}),
                        activity_definitions=(activity.to_dict() if activity else {}),
                ...
                for spend_point, raw_spend in enumerate(axis):
                    ...
                    delta = _finite_difference_delta(
                        derivative_axis_value, channel_support, marginal_delta
                    )
                    lower_bound = 0.0
                    upper_bound = np.inf
                    if channel_support["observed_support_status"] == SUPPORT_AVAILABLE:
                        lower_bound = float(channel_support["planning_spend_min"])
                        upper_bound = float(channel_support["planning_spend_max"])
                    lower_axis = max(lower_bound, derivative_axis_value - delta)
                    upper_axis = min(upper_bound, derivative_axis_value + delta)
                    if lower_axis == upper_axis:
                        raise ValueError(
                            "Cannot calculate a marginal derivative at a "
                            "zero-width governed support boundary"
                        )
                    if (
                        effective_curve_type == "monetary"
                        and not legacy_monetary
                        and cost_mapping is not None
                    ):
                        lower_spend = float(
                            cost_mapping.spend_to_media_input(lower_axis)
                        )
                        upper_spend = float(
                            cost_mapping.spend_to_media_input(upper_axis)
                        )
                    else:
                        lower_spend = lower_axis
                        upper_spend = upper_axis
                    ...
                    for outcome_id in meta.outcome_ids:
                        outcome_components = [
                            ...
                            if item.channel == channel and item.outcome_id == outcome_id
                        ]
                        ...
                        if (
                            ...
                            # Authoritative monetary derivative: perturb local
                            # spend, map both endpoints, predict, then divide by
                            # the reporting-currency change. This remains valid
                            # at piecewise cost knots.
                            marginal_reporting = (
                                (mu_upper[outcome_id] - mu_lower[outcome_id])
                                / ((upper_axis - lower_axis) * fx_rate)
                            ...
                            marginal_reporting = (
                                marginal_media_input / (spend_unit_scale * fx_rate)
                        ...
                        for component in outcome_components:
                            ...
                            component_eta.append(coefficient * strength * saturation)
                        ...
                        observed_status = channel_support["observed_support_status"]
                            ...
                            rows.append(
                                {
                                    ...
                                    **context.metadata(),
                                    ...
                                    "curve_type": effective_curve_type,
                                    "counterfactual_axis_type": (effective_curve_type),
                                    ...
                                    "media_input_unit": (
                                        input_spec.unit if input_spec else spend_unit
                                    ),
                                    ...
                                    "activity_definitions_fingerprint": (
                                        activity_definitions_fingerprint(activity_rows)
                                        if activity_rows
                                        else None
                                    ),
                                    "monetary_governance_fingerprint": (
                                    ...
                                    "marginal_incremental_response_per_media_input_unit": (
                                        marginal_media_input * eta_share
                                    ),
                                    ...
                                    "marginal_calculation_method": (
                                        ...
                                        if lower_axis == derivative_axis_value
                                        else (
                                            "backward_finite_difference"
                                            if upper_axis == derivative_axis_value
                                    ...
                                    "marginal_delta_reporting_spend": (
                                        delta * fx_rate
                                        if effective_curve_type == "monetary"
                                        else np.nan
                                    ),
                                    ...
                                    "current_spend": (
                                        channel_support["current_spend"]
                                        * (spend_unit_scale if legacy_monetary else 1.0)
                                        * fx_rate
                                        if observed_status == SUPPORT_AVAILABLE
                                    ...
                                    "current_reporting_spend": (
                                        channel_support["current_spend"] * fx_rate
                                        if observed_status == SUPPORT_AVAILABLE
                                        and effective_curve_type == "monetary"
                                        and not legacy_monetary
                                        else np.nan
                                    ...
                                    "support_axis_type": channel_support.get(
                                        "support_axis_type"
                                    ),
                                    "support_unit": channel_support.get("support_unit"),
                                    "support_provenance": channel_support.get(
                                        "support_provenance"
                                    ),
                                    "observed_media_input_min": (
                                        channel_support.get(
                                    ...
                                    "observed_reporting_spend_min": (
                                        channel_support["observed_spend_min"] * fx_rate
                                        if observed_status == SUPPORT_AVAILABLE
                                        and effective_curve_type == "monetary"
                                        and not legacy_monetary
                                        else np.nan
                                    ...
                                    "observed_reporting_spend_max": (
                                        channel_support["observed_spend_max"] * fx_rate
                                        if observed_status == SUPPORT_AVAILABLE
                                        and effective_curve_type == "monetary"
                                        and not legacy_monetary
                                        else np.nan
                                    ...
                                    "planning_reporting_spend_min": (
                                        channel_support["planning_spend_min"] * fx_rate
                                        if observed_status == SUPPORT_AVAILABLE
                                        and effective_curve_type == "monetary"
                                        and not legacy_monetary
                                        else np.nan
                                    ...
                                    "planning_reporting_spend_max": (
                                        channel_support["planning_spend_max"] * fx_rate
                                        if observed_status == SUPPORT_AVAILABLE
                                        and effective_curve_type == "monetary"
                                        and not legacy_monetary
                                        else np.nan
                                    ...
                                    **component_economics,
                                    ...
                                    "curve_attribution_reconciliation_error": (
                                        channel_incremental - matched_attribution
...
def aggregate_curve_draws(
    ...
    *,
    ...
    result = grouped.agg(
        ...
        is_extrapolated=(
            "is_extrapolated",
            lambda values: None if values.isna().all() else bool(values.dropna().max()),
        ),
        observed_support_status=(
            "observed_support_status",
            lambda values: values.iloc[0] if values.nunique() == 1 else SUPPORT_MISSING,
        ),
        counterfactual_prediction_reconciliation_error=(
            "counterfactual_prediction_reconciliation_error",
            "max",
    ...
    if values and "outcome_id" in result:
        ...
        result.loc[missing_value, "incremental_value"] = [
            row["incremental_response"] * values.get(row.get("outcome_id"), np.nan)
    ...
    ] = result.loc[cost_unavailable, "economics_availability_status"].replace(
        {
...
def aggregate_portfolio_marginal(
    ...
    *,
    ...
    missing = set(channel_draws["channel"]) - set(perturbation.allocation_direction)
    if missing:
        raise ValueError(
...
def reconcile_curve_to_attribution(
    draws: pd.DataFrame,
    attribution_by_market_channel_outcome: Mapping[Tuple[str, str, str], float],
) -> pd.DataFrame:
